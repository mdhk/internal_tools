...
# https://github.com/JAMJU/Sel_supervised_models_perception_biases/blob/main/CPC/cpc/model.py
#
...
class CPCEncoder(nn.Module):
    ...
    def forward(self, x, layer='conv5'):
        if layer == 'conv1':
...
class CPCModel(nn.Module):
    ...
    def forward(self, batchData, label, layer=None):
        cFeature = None
...
class ConcatenatedModel(nn.Module):
    ...
    def intermediate_forward(self, batchData, label, layer):

        outFeatures = []
        outEncoded = []
        for model in self.models:
            cFeature, encodedData, label = model(batchData, label, layer = layer)
            outFeatures.append(cFeature)
            outEncoded.append(encodedData)
        return torch.cat(outFeatures, dim=2), torch.cat(outEncoded, dim=2), label