...
class AudioPreprocessor:
    def __init__(self, *args, **kwargs):
        raise EnvironmentError(
            ...
            + f"{self.__class__.__name__}.for_hf_model(model_name_or_path), or one of the following: "
            + ", ".join(
                [
                    f"{self.__class__.__name__}.{m}()"
                    for m in [
                        d
                        for d in dir(AudioPreprocessor)
                        if d.startswith("for_") and d != "for_hf_model"
                    ]
                ]
            )
        )
    ...
    def for_hf_model(cls, model_name_or_path, *args, **kwargs):
        try:
            ...
            preprocessor = AutoFeatureExtractor.from_pretrained(
                "facebook/wav2vec2-base", *args, **kwargs
            )
...
class WaveformPreprocessor:
    ...
    def __call__(
        ...
        if padding:
            audio_signals = self._pad_to_max_length(
                audio_signals, padding_side=padding_side
            )
        ...
        if self.do_normalize:
            ...
            # if processing a batch of unpadded audio signals (signals do not have equal length)
                ...
                audio_signals = np.array(
                    [self._normalize(s).numpy() for s in audio_signals], dtype=object
                )

        return BatchFeature({"input_values": audio_signals})

    def __repr__(self):
        return f"{self.__class__.__name__} " + str(self.__dict__).replace(
            "{", "{\n "
        ).replace(",", ",\n").replace("}", "\n}")

    def _pad_to_max_length(self, audio_signals, padding_side: str = "right"):
    ...
    def _to_torch(self, audio_signals: ArrayLike):
        return torch.as_tensor(audio_signals, dtype=torch.float32)

    def _normalize(self, audio: torch.Tensor | np.ndarray):
...
class MelPreprocessor:
    ...
    def __call__(
        ...
        mel_feats = [
            self._extract_fbank(
                signal, self.norm_mean, self.norm_std, fp=self.frame_period
            )
        ...
        if padding:
            ...
            mel_feats = np.array(
                [mf.detach().cpu().numpy() for mf in mel_feats], dtype=object
            )
        ...
        if self.return_attention_mask:
            preproc_features = BatchFeature(
                {"input_values": mel_feats, "attention_mask": pad_mask}
            )
        else:
    ...
    def __repr__(self):
        ...
        return f"{self.__class__.__name__} " + str(config_dict).replace(
            "{", "{\n "
        ).replace(",", ",\n").replace("}", "\n}")
    ...
    def _extract_fbank(self, waveform, mean, std, fp=20):
        ...
        if fp == 20:
            ...
            if odd_y.shape[0] != even_y.shape[0]:
                even_y = torch.cat(
                    (even_y, torch.zeros(1, even_y.shape[1]).to(y.device)), dim=0
                )
